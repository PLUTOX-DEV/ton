...
def telegram_login(request):
    ...
    # Step 1: Check if authentication is expired (24 hours)
    ...
    logger.debug("Received hash: %s", received_hash)

    if calculated_hash != received_hash:
        return Response({"error": "Invalid data. Hash mismatch."}, status=400)

    # Step 3: Create or update TelegramUser
    ...
    # Step 4: Sync with Django's User model
    username = f"tg_{telegram_user.telegram_id}"
    user, _ = User.objects.get_or_create(username=username)

    # Optional: Prevent traditional login
...
def get_jwt_token(request):
    """
    Traditional login endpoint for Django users using username & password.
    """
